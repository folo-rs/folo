...
//!
//! This is enabled by `linked::Box` which works like `std::boxed::Box` but with the
...
linked::variable!(static RECORDS_PROCESSED: linked::Box<dyn Counter> = EventCounter::new_as_counter());
