//! An asynchronous event library that provides one-shot event delivery.
//!
...
/// Smart pointer implementation for reference-counted values in slabs.
///
/// This module provides the foundation for managing memory-efficient, reference-counted
/// storage within the `OnceEvent` implementation.
pub mod pinned_rc;
mod with_ref_count;
...
impl<T> OnceEvent<T> {
    ...
    pub unsafe fn new_embedded(
        ...
        // SAFETY: The caller guarantees that the storage is valid and pinned.
        let storage_ref = unsafe { &mut *storage.inner.get() };
        *storage_ref = with_ref_count;

        // SAFETY: The caller guarantees that the storage outlives all references.
        let event = unsafe { Pin::into_inner_unchecked(storage) };
