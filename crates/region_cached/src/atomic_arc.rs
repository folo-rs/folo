...
/// Stores an Arc and provides atomic operations on the reference itself.
#[derive(Debug)]
...
impl<T> AtomicArc<T> {
    pub fn new(value: Arc<T>) -> Self {
        Self {
            inner: AtomicPtr::new(Arc::into_raw(value).cast_mut()),
...
impl<T> Drop for AtomicArc<T> {
    fn drop(&mut self) {
        ...
        // The Arc must also be valid - it is, because we are still holding ownership via `inner`.
        unsafe {
            Arc::decrement_strong_count(self.inner.load(atomic::Ordering::Relaxed));
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn smoke_test() {
        let value = Arc::new(43);
        let atomic = AtomicArc::new(Arc::clone(&value));

        assert_eq!(atomic.load(), value);

        let value = Arc::new(42);
        atomic.store(value.clone());
    ...
    #[test]
    fn test_drop() {
        let atomic = AtomicArc::new(Arc::new(42));
