...
# Resolves the built `cargo-bench-history-faker` test-support binary from `cargo build` JSON output.
#
# The benchmark faker lives in its own lib+bin package (packages/cargo-bench-history-faker),
# so Cargo does not expose it to the cargo-bench-history integration tests via CARGO_BIN_EXE_*.
# A test runner builds it once up front and passes the path via CBH_FAKER so each per-test
...
function Resolve-FakerExecutable {
    # Scans the JSON lines emitted by `cargo build --message-format=json` and returns the path to
    # the `cargo-bench-history-faker` binary (the last matching compiler-artifact, mirroring cargo's
    # own "last one wins" ordering). Non-JSON lines (e.g. rendered diagnostics) and messages that
    # carry no target/executable are skipped without error, so it is safe under Set-StrictMode.
    # Throws when no matching executable is found, so a silently-missing build surfaces immediately.
    [CmdletBinding()]
    [OutputType([string])]
    param(
    ...
    foreach ($line in $CargoMessage) {
        ...
        if (-not (Test-HasProperty $message.target 'name') -or $message.target.name -ne 'cargo-bench-history-faker') { continue }

        $exe = $message.executable
    }

    if (-not $exe) {
        throw 'could not resolve the cargo-bench-history-faker executable from cargo output'
    }
    return $exe
}
...
Export-ModuleMember -Function Resolve-FakerExecutable
