...
fn main() {
    println!("=== OpaquePool Partial Initialization Examples ===");
    println!();
...
fn demonstrate_partial_buffer() {
    ...
    let mut pool = OpaquePool::builder().layout_of::<PartialBuffer>().build();
    ...
    // SAFETY: PartialBuffer matches the layout and we properly initialize required fields.
    let buffer = unsafe {
...
fn demonstrate_cache_entry() {
    ...
    let mut pool = OpaquePool::builder().layout_of::<CacheEntry>().build();

    // Create cache entries with only keys, values computed on demand
    // SAFETY: CacheEntry matches the layout and we initialize the key.
    let entry1 = unsafe {
        pool.insert_with(|uninit| {
    ...
    pool.remove_mut(entry1);
    pool.remove_mut(entry2);
    println!();
}
