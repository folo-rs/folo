...
impl Groups {
    ...
    pub fn group_of(&self, package: &str) -> Option<&str> {
    ...
    #[must_use]
    pub fn closure(&self, package: &str) -> Vec<String> {
...
mod tests {
    ...
    use super::*;
    use crate::classify::fixture::classification;

    #[test]
    fn workspace_groups_follow_acquired_edges_and_include_private_members() {
        let mut workspace = classification(Vec::new()).work_tree;
        workspace.version_targets = ["public", "helper", "loose"]
            .into_iter()
            .map(|name| VersionTarget {
                name: name.to_owned(),
                version: v("1.2.3"),
            ...
            .collect();
        workspace.exact_dependencies.push(ExactDependency {
            source: "public".to_owned(),
            target: "helper".to_owned(),
            requirement: "=1.2.3".to_owned(),
            manifest_path: "public/Cargo.toml".into(),
            location: "dependencies.helper".to_owned(),
        });
        let groups = Groups::from_workspace(&workspace);
        assert_eq!(groups.members("helper"), ["helper", "public"]);
        assert_eq!(groups.group_of("public"), Some("helper"));
        assert_eq!(groups.group_of("helper"), Some("helper"));
        assert_eq!(groups.group_of("loose"), None);
    }
