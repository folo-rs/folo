...
pub fn run_analysis_order(path: &Path, verbose: Verbose<'_>) -> Result<String, AppError> {
...
fn analysis_order(report: &ReportFile, verbose: Verbose<'_>) -> Vec<AnalysisBatch> {
...
mod tests {
    ...
    fn batches(report: &ReportFile) -> Vec<(Vec<String>, bool)> {
        analysis_order(report, Verbose::new(false, &crp_diag::Discard))
            .into_iter()
            .enumerate()
            .map(|(index, batch)| {
    ...
    fn independent_names_are_ordinal_and_input_order_independent() {
        ...
        assert_eq!(
            serde_json::to_string(&analysis_order(
                &graph(&[]),
                Verbose::new(false, &crp_diag::Discard)
            ))
            .unwrap(),
