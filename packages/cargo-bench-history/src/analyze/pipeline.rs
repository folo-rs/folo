//! Implementation of the `analyze` pipeline: the orchestration entry points
//! ([`execute`], [`analyze_with`]) and the pure selection, window-resolution,
//! parsing, and reporting logic they compose. The parent module re-exports the
//! surface the sibling query commands (`list`, `prune`, `examine`, `bless`) share.
...
use cargo_bench_history_core::analyze::{
    ...
    DiscriminantSetQuery, FacetFilter, ReportInput, RunPoints, Series, SeriesBuilder, SeriesFilter,
    SetSummary, StorageKey, apply_blessings, balanced_chunk_sizes, find_changes_spawned, parse_key,
    render, render_markdown_summary, select_commits, worker_count,
};
