...
|---|---|---|
| discriminant set mismatch | `Warning: comparison base is 7 commits behind base (discriminant set mismatch)` | Newer base data exists, but it was measured under a different machine key. Measurements are never compared across machine keys, so the comparison reached back to the newest base commit this partition covers. A rotating CI pool is the usual cause. |
