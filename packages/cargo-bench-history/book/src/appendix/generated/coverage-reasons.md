...
|---|---|---|
| `ghost` | A series not measured at the analyzed tip commit. | Nothing, if the benchmark was removed or its package was not built. Otherwise check that it still runs at the analyzed commit. |
...
| `too_few_base_commits` | A series with too few base-branch commits to compare against. | Measure more of the base branch. A comparison needs a base window to compare against. |
