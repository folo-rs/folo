...
|---|---|---|
| `no_series` | Nothing was analyzed, so no change could be detected. | Nothing at all. The run is a collection failure, not a result: treat it as a broken run rather than as an all-clear. |
