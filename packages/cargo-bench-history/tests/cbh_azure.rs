...
async fn analyze_with_cache_round_trips_through_real_azure() {
    ...
    with_real_azure_container(async |container| {
        scenario_cache_round_trip(AzureWorkspace::new(&real_azure_config(&container))).await;
    })
    .await;
}
...
async fn prune_without_cache_round_trips_through_real_azure() {
    ...
    with_real_azure_container(async |container| {
        scenario_prune_without_cache(AzureWorkspace::new(&real_azure_config(&container))).await;
    })
    .await;
}
