...
/// own commits. The same workflow verifies checkout preservation and that analyze
/// can read every backfilled object, without rebuilding a separate linear history.
#[tokio::test]
#[cfg_attr(miri, ignore)]
async fn backfill_spans_first_parent_merge_and_restores_checkout() {
    ...
    // Derive host provenance from the real backfill result so key checks are portable.
    ...
    assert_eq!(
        parsed["runs"], 3,
        "analyze should see every backfilled first-parent commit: {report}"
    );
}
