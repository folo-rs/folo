...
/// they keep this alive, so they share it. Both reach it through a mutex, which also covers the
/// diagnostic registry - renting an event therefore registers it without taking a second lock.
///
...
pub(crate) struct PoolState<T: 'static> {
    ...
    #[cfg(debug_assertions)]
    registry: EventRegistry<UnsafeCell<Event<T>>>,
}
...
impl<T: Send + 'static> PoolState<T> {
    ...
    ///
    /// The caller becomes responsible for passing the pointer to [`destroy_event()`] exactly
    /// once, having first deregistered it via [`unregister()`][Self::unregister].
    #[must_use]
    #[cfg_attr(
    ...
    pub(crate) fn rent(&mut self) -> NonNull<UnsafeCell<Event<T>>> {
    ...
    #[cfg(debug_assertions)]
    pub(crate) fn unregister(&mut self, event: NonNull<UnsafeCell<Event<T>>>) {
    ...
    pub(crate) fn awaiter_backtraces(&self) -> Vec<Arc<Backtrace>> {
        ...
        for event in self.registry.iter() {
            ...
            // requires the exclusive access to the registry that our caller's lock excludes, so
            // every event still named here is initialized, aligned and not yet destroyed.
...
/// # Safety
///
/// The pointer must come from [`PoolState::rent()`] and must not have been passed here before.
