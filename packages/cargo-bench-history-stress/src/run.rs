...
mod tests {
    ...
    async fn branch_measurement_handles_more_base_evidence_than_its_window_cap() {
        ...
        let sets: Vec<_> = scenario::discriminant_sets()
            .into_iter()
            .filter(|set| set.target_triple.as_str() == TARGET)
            .collect();
        assert_eq!(
            sets.iter().map(|set| set.engine).collect::<Vec<_>>(),
            Engine::ALL
        );
        ...
        let (main_times, feature_times) =
            scenario::commit_times(anchor, scenario.commits, scenario.branch_commits);
        let clock = analysis_clock(&main_times, &feature_times).unwrap();
        ...
        .await
        .unwrap();
        write_config(workspace_dir.path(), &target, logger)
            .await
            .unwrap();
        seed::seed(target.seed_root(), scenario, &sets, &repo, logger).unwrap();
        ...
        assert_eq!(
            measured.runs,
            (with_runs + BRANCH_COMMITS + DIRTY_RUNS) * sets.len()
        );
        assert_eq!(measured.series, BENCHMARKS * sets.len());
        // The drifting and blessable families are elevated on the feature branch.
        assert_eq!(measured.regressions, 2 * sets.len());
