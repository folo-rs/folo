...
Subprocess unit tests use repository-independent Git operations in disposable
directories to cover successful output capture and nonzero exits. Tests that need
repository state create it explicitly through the hermetic integration fixture;
the source tree's Git metadata is never a test prerequisite.
