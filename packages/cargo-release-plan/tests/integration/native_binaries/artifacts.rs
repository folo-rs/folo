...
use crate::native_binaries::{Fixture, SMOKE_WATCHDOG, assert_success, compile_tool, run};
...
"#,
        ...
        let mut metadata: Value = serde_json::from_str(&run(
            fixture.root.path(),
            "cargo",
            &[
                "metadata",
        ...
        assert!(
            !fixture
                .root
                .path()
                .join("out/relative-target/outcome.json")
                .exists()
        );
    });
}
...
fn assert_failed_build(fixture: &Fixture, result: &Output, name: &str) {
    ...
    let outcomes = fixture.outcomes(&format!("out/{name}"));
