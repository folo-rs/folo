...
pub struct PackageManifest {
    ...
    pub inherited_path_dependencies: Vec<String>,
    /// Effective normal and build declarations used to filter locked workspace edges.
    pub installation_dependencies: InstallationDependencies,
    ...
    pub inherited_resource_paths: Vec<String>,
...
#[derive(Clone, Debug)]
pub enum InstallationDependencies {
...
pub struct DependencyPatch {
    pub origin: String,
    pub name: String,
    pub replacement: Result<InstallationDependency, InstallationError>,
}
...
impl PathCase {
    ...
    pub fn recorded_path<'a>(self, paths: &'a [String], requested: &str) -> Option<&'a str> {
        paths
            .iter()
    ...
    pub fn same_path(self, left: &str, right: &str) -> bool {
        match self {
...
pub fn parse_document(path: &Path, content: &str) -> Result<DocumentMut, AppError> {
    content
        .parse()
...
pub const DEFAULT_README_FILES: &[&str] = &[PRIMARY_README, "README.txt", "README"];
...
pub struct WorkspaceInherit<'a> {
    pub package: Option<&'a dyn TableLike>,
    pub dependencies: Option<&'a dyn TableLike>,
}

impl fmt::Debug for WorkspaceInherit<'_> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        // TableLike has no Debug contract; report which inheritance sources are available.
...
impl<'a> WorkspaceInherit<'a> {
    pub fn from_root(root: &'a DocumentMut) -> Self {
    ...
    pub fn dependency(&self, name: &str) -> Option<&'a dyn TableLike> {
...
pub fn installation_dependencies(
    doc: &DocumentMut,
    workspace: &WorkspaceInherit<'_>,
    path: &Path,
...
pub fn installation_patches(root: &DocumentMut) -> Vec<DependencyPatch> {
...
pub fn collect_registry_indices(doc: &DocumentMut, indices: &mut BTreeMap<String, String>) {
...
pub fn for_each_dependency_table(
...
pub fn dependency_table_name<'a>(table: &dyn TableLike, name: &'a str) -> Option<&'a str> {
...
pub const DEPENDENCY_TABLES: &[&str] = &["dependencies", "dev-dependencies", "build-dependencies"];
...
pub struct MemberPattern {
    pub literal: String,
    pub case: PathCase,
    pub matcher: Override,
}

impl MemberPattern {
    pub fn new(pattern: &str, case: PathCase) -> Result<Self, AppError> {
