...
pub struct WorkTree {
    ...
    pub exact_dependencies: Vec<ExactDependency>,
    ...
    pub members_by_dir: BTreeMap<PathBuf, String>,
    /// Normal and build declarations, including non-publishable tracked members.
    pub installation: InstallationGraph,
}
...
impl ManifestSnapshot {
    ...
    #[must_use]
    pub fn root(&self, workspace_root: &Path) -> &DocumentMut {
    ...
    #[must_use]
    pub fn package(&self, path: &Path) -> Option<&PackageManifest> {
        self.packages
            .get(path)
...
pub fn capture_metadata(manifest_path: &Path) -> Result<String, AppError> {
