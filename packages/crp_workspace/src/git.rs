...
impl GitRepo {
    ...
    pub fn discover(start: &Path) -> Result<Self, AppError> {
...
impl WorkTreeModes {
    ...
    pub fn set(&mut self, path: &str, mode: &str) {
        match mode {
            EXECUTABLE_TREE_MODE => {
...
mod tests {
    ...
    fn tree_records_report_the_executable_bit() {
        ...
        let record = format!("{} blob {}\t{}", script.mode, script.id, script.path);
        assert!(TreeEntry::parse(&record).unwrap().is_executable());
