...
//! thread-safe primitive and contains hand-written regression tests
//! for known-tricky reentrancy scenarios.
//!
//! These tests cover **single-threaded reentrancy**, not concurrent
//! ordering. Multithreaded race coverage for `ManualResetEvent`
//! lives in the dedicated tests in [`crate::manual`] alongside the
//! Miri-driven test hooks; this pilot complements but does not
//! replace those.
