...
fn entrypoint(c: &mut Criterion) {
    ...
    let mut g = c.benchmark_group("linked_static_thread_local_rc/access_one_threaded");
    ...
    Run::new()
        .iter(|_| Arc::weak_count(&TARGET.to_rc().shared_state))
        .execute_criterion_on(&mut one_thread, &mut g, "to_rc");
    ...
    if let Some(ref mut two_threads) = two_threads {
        let mut g = c.benchmark_group("linked_static_thread_local_rc/access_two_threaded");
        ...
        Run::new()
            .iter(|_| Arc::weak_count(&TARGET.to_rc().shared_state))
            .execute_criterion_on(two_threads, &mut g, "to_rc");
