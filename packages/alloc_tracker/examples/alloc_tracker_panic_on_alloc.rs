...
//! ```bash
//! cargo run --example alloc_tracker_panic_on_alloc --features panic_on_next_alloc
//! ```
...
fn main() {
    ...
    println!(
        "Run with: cargo run --example alloc_tracker_panic_on_alloc --features panic_on_next_alloc"
    );
}
