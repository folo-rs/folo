    ...
    group.bench_function("traditional_full_init", |b| {
        b.iter_custom(|iters| {
            ...
            for i in 0..iters {
                ...
                let handle = black_box(pool.insert(item));
                // SAFETY: The pointer is valid and the memory contains the value we just inserted.
                black_box(unsafe { handle.ptr().as_ref() }.capacity());
                // SAFETY: We can safely remove the item we just inserted.
                unsafe {
                    pool.remove(&handle);
                }
            }
    ...
    group.bench_function("partial_init_insert_with", |b| {
        b.iter_custom(|iters| {
            ...
            for i in 0..iters {
                ...
                // SAFETY: We properly initialize the PartialBuffer in the closure.
    ...
    group.bench_function("cache_traditional", |b| {
        b.iter_custom(|iters| {
            ...
            for i in 0..iters {
                ...
                let handle = black_box(pool.insert(item));
                // SAFETY: The pointer is valid and the memory contains the value we just inserted.
                black_box(unsafe { handle.ptr().as_ref() }.key());
                // SAFETY: We can safely remove the item we just inserted.
                unsafe {
                    pool.remove(&handle);
                }
    ...
    group.bench_function("cache_partial_init", |b| {
        b.iter_custom(|iters| {
            ...
            for i in 0..iters {
                ...
                let key = format!("key_{}", i as usize);
                // SAFETY: We properly initialize the CacheEntry in the closure.
                let handle = black_box(unsafe {
                    pool.insert_with(|uninit| {
