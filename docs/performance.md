...
principles for their domain — for example, `packages/events_once/AGENTS.md`
codifies the relative priority of pooled, embedded and boxed events, the
expectation that `LocalEvent` beats `Event`, and the unusual sensitivity of the
event machinery to `#[inline]` placement. Always check for a package-local
