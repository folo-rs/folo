...
it needs exactly this one-time bootstrap: publish `0.0.5` manually with `cargo
publish -p cargo-bench-history-faker`, then configure its Trusted Publisher on
crates.io (owner `folo-rs`, repo `folo`, workflow `release.yml`) so every
subsequent release goes through CI like the rest of the family. The other
