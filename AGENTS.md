...
`UnwindSafe` and `RefUnwindSafe` are NOT unsafe traits. Manual implementations use plain `impl`,
not `unsafe impl`.

When a `!Sync` marker is needed, use `PhantomData<*mut ()>` with `unsafe impl Send` rather than
`PhantomData<Cell<()>>`. The `Cell<()>` approach breaks `RefUnwindSafe` because `Cell` contains
`UnsafeCell` which is `!RefUnwindSafe`. The `*mut ()` approach achieves the same `!Sync` effect
while preserving unwind safety.
...
`T: RefUnwindSafe` and trait objects are `!RefUnwindSafe`. Since ohno error types are immutable
after construction, add `impl UnwindSafe` and `impl RefUnwindSafe` for them.
