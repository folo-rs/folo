...
types are defined with `ohno` (private condition leaves, public library aggregates,
application `AppError` boundaries, narrow caller decisions, constructor visibility,
`#[display]` scoping, which derives survive).
